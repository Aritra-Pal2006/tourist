...
5. Configure the service:
   ...
   - Build Command: `cd client && npm install && npm run render-build`
...
1. **ES Module Error**: If you see "ReferenceError: require is not defined in ES module scope":
   - This is handled by using proper ES module syntax in all JavaScript files
   - The project is configured with `"type": "module"` in [package.json](file:///d:/projects/tourist/client/package.json)
...
3. **General Build Failures**:
   ...
   - Check that the `NODE_VERSION` environment variable is set to `18`
   - If issues persist, you can try changing the build command to:
